...
fun RegionScreen(
    ...
    val grouped = filterSearchViewModel.uiState.listAvailableRegion.groupBy { it.first() }
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                    TextField(
                        value = filterSearchViewModel.uiState.searchValueRegion,
                        onValueChange = {
                            filterSearchViewModel.event(Event.SearchRegions(it))
                        },
                        singleLine = true,
            ...
            grouped.forEach { (key, value) ->
                ...
                items(value, key = { item -> item  }) {
                    Surface(
                        ...
                        onClick = {
                            filterSearchViewModel.event(Event.SelectRegion(it))
                            onBack()
                        },
