...
fun UniversitiesScreen(
    ...
    val grouped = uiState.listAvailableNameUniversity.groupBy { it.first() }
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                    TextField(
                        value = filterSearchViewModel.uiState.searchNameUniversity,
                        onValueChange = {
                            filterSearchViewModel.event(Event.SearchUniversity(it))
                        },
                        singleLine = true,
            ...
            grouped.forEach { (key, value) ->
                ...
                items(value, key = { item -> item  }) {
                    Surface(
                        ...
                        onClick = {
                            filterSearchViewModel.event(Event.SelectUniversity(it))
                            onBack()
                        },
